...
void simplex::calc(){
        ...
        if(second_flag) { //2段階
                ...
                //必要なデータを入力
                lp2.obj = MAXIMIZE; //目的
                lp2.second_step = true; //2段階フラグ
                copy(restriction.begin(), restriction.end(), back_inserter(lp2.restriction)); //制約コピー
                copy(con_num.begin(), con_num.end(), back_inserter(lp2.con_num)); //定数項コピー
                copy(base.begin(), base.end(), back_inserter(lp2.base)); //基底変数コピー

                for(int i = 0; i < lp2.con_num.size(); ++i) {
                        if(lp2.con_num[i] > 0) { //定数が正の場合
                                ++lp2.sub; //補助変数カウント
                                string data = "w_" + to_string(lp2.sub); //補助変数名
                                lp2.obj_func[data] = -1; //目的関数の補助変数係数
                                lp2.base.emplace_back(data); //基底名記憶
                                lp2.restriction[i-1][data] = 1; //制約式に係数格納
                        }
                        if(lp2.con_num[i] < 0) { //定数が負の場合
                                ++lp2.sub; //補助変数カウント
                                string data = "w_" + to_string(lp2.sub); //補助変数名
                                lp2.obj_func[data] = -1; //目的関数の補助変数係数
                                lp2.base.emplace_back(data); //基底名記憶
        ...
        if(!second_step) {
                for(int i = 0; i < slack; ++i) base.pop_back();
                for(int i = 1; i < base.size(); ++i) {
                        ans[base[i]] = 0;
                        for(int j = 1; j < tab.size(); ++j) {
                                if(tab[j][i] == 1)
